...
physics_alpha = 0.1  # weight for monotonicity penalty
physics_beta = 0.0   # optional curvature smoothing


def physics_loss(y_true, y_pred):
    mse = K.mean(K.square(y_true - y_pred))
    diffs = y_pred[1:] - y_pred[:-1]
    mono_penalty = K.mean(K.relu(diffs))  # penalize upward jumps
    if physics_beta > 0:
        ddiffs = diffs[1:] - diffs[:-1]
        smooth_penalty = K.mean(K.square(ddiffs))
    else:
        smooth_penalty = 0.0
    return mse + physics_alpha * mono_penalty + physics_beta * smooth_penalty
...
#############777777777777777777777777777
    ...
    # ############## keep temporal order for physics loss (no shuffle)
    ...
    if __name__ == '__main__':
        ...
        model_dir = os.path.join(last_path, "model", "FCN_RUL_1out_train_valid_test")
        os.makedirs(log_dir, exist_ok=True)
        os.makedirs(err_dir, exist_ok=True)
        os.makedirs(model_dir, exist_ok=True)
        
        if os.path.exists(os.path.join(err_dir, f"{method_name}.txt")):
            os.remove(os.path.join(err_dir, f"{method_name}.txt"))
    ...
    #######             single  output                
        ...
        for i in range(run_times):
            ...
            modelcheckpoint=keras.callbacks.ModelCheckpoint(monitor='loss',filepath=os.path.join(model_dir, f"{model_name}.h5"),save_best_only=True,verbose=1)
            hist = model.fit(X_train, Y_train, batch_size=batch_size, epochs=nb_epochs,
                      verbose=1, validation_data=(X_test, Y_test), callbacks = [reduce_lr,earlystopping,modelcheckpoint], shuffle=False)   
  
            log = pd.DataFrame(hist.history)
            log.to_excel(os.path.join(log_dir, "{}_dataset_{}_log{}_time{}.xlsx".format(method_name,dataset,i,datetime.datetime.now().strftime('%Y%m%d%H%M%S'))))
            ...
            plt.legend()
            plt.show()
            model=keras.models.load_model(os.path.join(model_dir, f"{model_name}.h5"),custom_objects={'root_mean_squared_error': root_mean_squared_error})
            ...
            file = open(os.path.join(err_dir, f"{method_name}.txt"), 'a')
